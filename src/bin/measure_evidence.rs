...
fn main() {
    ...
    let frame = match image::open(frame_path) {
        Ok(img) => {
            println!(
                "✅ [MEASURED] Frame loaded successfully from {}",
                frame_path
            );
            img.to_rgba8()
        }
    ...
    println!(
        "✅ [MEASURED] Frame resolution: {}x{} pixels",
        frame_w, frame_h
    );
    ...
    for iteration in 0..100 {
        ...
        let game_state =
            GameState::from_world_state(world_state, iteration as u64, frame_w, frame_h);
    ...
    println!(
        "✅ [MEASURED] Pretty JSON size: {} bytes",
        json_pretty.len()
    );
    println!(
        "✅ [MEASURED] Compact JSON size: {} bytes",
        json_compact.len()
    );
    ...
    if state.hud.hp.confidence > 0.0 || state.hud.mp.confidence > 0.0 {
        println!(
            "✅ [MEASURED] HUD Detector: Active (confidence: HP={:.2}, MP={:.2}, EXP={:.2})",
    ...
    println!(
        "✅ [MEASURED] 30 FPS (33.33ms budget):  {}/{} frames = {:.1}%",
        within_30,
        timings.len(),
        within_30 as f32 / timings.len() as f32 * 100.0
    );
    println!(
        "✅ [MEASURED] 50 FPS (20.00ms budget):  {}/{} frames = {:.1}%",
        within_50,
        timings.len(),
        within_50 as f32 / timings.len() as f32 * 100.0
    );
    println!(
        "✅ [MEASURED] 60 FPS (16.67ms budget):  {}/{} frames = {:.1}%",
        within_60,
        timings.len(),
        within_60 as f32 / timings.len() as f32 * 100.0
    );
    ...
    println!(
        "  Timings CSV: {}/csv/latency_measurements.csv",
        evidence_dir
    );
    println!(
        "  Analysis:    {}/benchmarks/realtime_analysis.txt",
        evidence_dir
    );
